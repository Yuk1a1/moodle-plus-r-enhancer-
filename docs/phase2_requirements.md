...
`core_course_get_contents` APIが返す `contents[].fileurl` にはすでに `pluginfile.php` 形式のURLが含まれている。これに `?forcedownload=1` を付与するだけで、クリック時にブラウザのダウンロードマネージャが起動する。

```javascript
// fileurl例: https://lms.ritsumei.ac.jp/webservice/pluginfile.php/.../file.pdf
// content script からの場合は通常の pluginfile.php を使う
const downloadUrl = module.contents[0].fileurl;
// forcedownload パラメータ付与
const forceUrl = downloadUrl.includes('?')
